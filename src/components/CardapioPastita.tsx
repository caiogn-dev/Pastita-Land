...
function CategoryPill({ label, active, onClick }: { label: string; active?: boolean; onClick?: () => void; }) {
  return (
    <button
      onClick={onClick}
      className={`whitespace-nowrap rounded-full px-4 py-2 text-sm transition border ${active ? "bg-rose-600 text-white border-rose-600" : "bg-white/80 hover:bg-white border-zinc-200"}`}
    >
      {label}
    </button>
  );
    ...
    <div className="group overflow-hidden rounded-2xl border border-zinc-200 shadow-sm bg-white">
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span className="font-semibold text-rose-700">{item.price.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
        </div>
        {item.description && <p className="mt-2 text-sm text-zinc-600 line-clamp-3">{item.description}</p>}
        {item.tags && item.tags.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-2">
            {item.tags.map((t) => (
              <span key={t} className="text-xs px-2 py-1 rounded-full bg-rose-50 text-rose-700 border border-rose-100">{t}</span>
            ))}
          </div>
        )}
        <div className="mt-4">
...
function CartButtonFloating({ onClick, itemCount }: { onClick: () => void; itemCount: number; }) {
  return (
    ...
    >
      ...
      <span>Carrinho</span>
      {itemCount > 0 && (
        <span className="ml-2 bg-white text-rose-700 rounded-full px-2 py-0.5 text-xs font-bold min-w-[24px] text-center">{itemCount}</span>
      )}
    </button>
  );
}
...
function buildOrderBlock(items: any[], total: number) {
  const lines = [
    ...
    "",
    ...items.map((it) => `• ${it.qty}x ${it.name} — ${(it.price).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })} (cada)`),
    "",
...
function CartModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const whatsappHref = useMemo(() => {
    return `https://wa.me/${WHATS_PHONE}?text=${encodeURIComponent(block)}`;
  }, [block]);
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-6">
            ...
            {items.map((it) => (
              <div key={it.id} className="flex items-center justify-between gap-4 p-4 hover:bg-zinc-50 transition">
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-zinc-500">{it.qty} x {(it.price).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-base font-bold text-rose-700 min-w-[70px] text-right">{(it.price * it.qty).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
                  <button onClick={() => remove(it.id)} className="rounded-full bg-rose-100 hover:bg-rose-200 text-rose-700 px-3 py-1 text-xs font-semibold transition border border-rose-200 shadow-sm" title="Remover item">Remover</button>
                </div>
              </div>
            ))}
          </div>
          <div className="flex items-center justify-between px-1">
            ...
            <span className="text-2xl font-bold text-rose-700">{total.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</span>
          </div>
          <div className="space-y-2">
            <label htmlFor="order-block" className="text-sm text-zinc-700 font-medium">Resumo do pedido (copiável):</label>
            <textarea id="order-block" className="w-full rounded-xl border border-zinc-200 p-3 text-base bg-zinc-50 focus:bg-white focus:ring-2 focus:ring-rose-200 transition" rows={5} readOnly value={block} />
          </div>
          <div className="flex flex-col sm:flex-row gap-2 mt-2">
            <button onClick={copyBlock} className="flex-1 rounded-xl bg-zinc-900 text-white hover:bg-zinc-800 py-3 text-base font-semibold shadow transition">Copiar resumo</button>
            <a href={whatsappHref} target="_blank" rel="noopener noreferrer" onClick={onFinishClick} className="flex-1 text-center rounded-xl bg-rose-600 text-white hover:bg-rose-700 py-3 text-base font-semibold shadow transition">Finalizar pelo WhatsApp</a>
            <button onClick={clear} className="rounded-xl bg-zinc-100 hover:bg-zinc-200 text-zinc-700 px-4 py-3 text-base font-semibold border border-zinc-200 shadow transition">Limpar</button>
          </div>
        </div>
      )}
    </Modal>
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute inset-x-0 top-10 mx-auto w-full max-w-2xl rounded-2xl bg-white shadow-xl border border-zinc-200">
        <div className="flex items-center justify-between p-4 border-b border-zinc-200">
          ...
          <button onClick={onClose} className="rounded-lg border border-zinc-200 px-3 py-1 text-sm">Fechar</button>
        </div>
...
export default function CardapioPastita() {
  ...
  const flatItems = useMemo(() => categories.flatMap((c) => c.items.map((i) => ({ ...i, __cat: c.title, __catId: c.id } as any))), [categories]);
  ...
  const visibleCategories = useMemo(() => activeCat === "todos" ? categories : categories.filter((c) => c.id === activeCat), [categories, activeCat]);
  ...
  return (
    <main className="min-h-screen bg-gradient-to-b from-rose-50 to-white">
      ...
      <div className="fixed z-40 bottom-24 right-6 sm:right-8">
        <button onClick={() => setOpenCombo(true)} className="bg-rose-600 hover:bg-rose-700 text-white rounded-full shadow-lg px-6 py-3 font-bold text-base border-2 border-white">Montar Combo</button>
      </div>
      ...
      <header className="sticky top-0 z-30 bg-rose-700 border-b border-rose-800 shadow-md">
        <div className="mx-auto max-w-6xl px-4">
          <div className="flex items-center justify-between gap-4 py-3">
            <div className="flex items-center gap-3">
              <div className="h-10 w-auto"><PastitaLogo /></div>
              <div className="ml-2">
                <h1 className="text-lg font-semibold text-white drop-shadow">Cardápio</h1>
                <p className="text-xs text-zinc-200">Escolha seus pratos favoritos e monte seu pedido!</p>
              </div>
            </div>
            <div className="w-full max-w-sm">
              <input type="search" placeholder="Buscar no cardápio..." value={query} onChange={(e) => setQuery(e.target.value)} className="w-full rounded-xl border border-rose-200 bg-white/90 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-rose-200" />
            </div>
          </div>
        </div>
      </header>
      <div className="mx-auto max-w-6xl px-4">
        <div className="flex gap-2 overflow-x-auto py-4">
          ...
          {categories.map((c) => (
            <CategoryPill key={c.id} label={c.title} active={activeCat === c.id} onClick={() => setActiveCat(c.id)} />
          ))}
        </div>
      </div>
      <section className="mx-auto max-w-6xl px-4 pb-16">
          ...
          return (
            <div key={category.id} className="py-6" id={category.id}>
              <div className="flex items-baseline justify-between mb-4">
                ...
                <a href={`#${category.id}`} className="text-sm text-rose-700 hover:underline">Ir para seção</a>
              </div>
        ...
        {filtered.length === 0 && (
          <div className="py-20 text-center text-zinc-600">Nenhum item encontrado para "{query}".</div>
        )}
      </section>
      <footer className="border-t border-zinc-200 bg-white/70">
        <div className="mx-auto max-w-6xl px-4 py-8 text-sm text-zinc-600">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            <p>© {new Date().getFullYear()} Pastita Massas — Template Next.js. Atualize as fotos e valores quando quiser.</p>
            <a href="/" className="hover:underline text-rose-700">Voltar ao início</a>
          </div>
        </div>
      </footer>
    </main>
